...
move2kube transform --config ../e2etest-1-language-platforms-m2kconfig.yaml --qa-skip || exit
ls
ls myproject
...
kubectl wait --for=condition=ready pod --timeout=90s --selector=move2kube.konveyor.io/service=golang || exit
kubectl get pods
echo 'after golang pods come online'
kubectl wait --for=condition=ready pod --timeout=90s --selector=move2kube.konveyor.io/service=nodejs || exit
kubectl get pods
echo 'after nodejs pods come online'
kubectl wait --for=condition=ready pod --timeout=90s --selector=move2kube.konveyor.io/service=ruby || exit
kubectl get pods
echo 'after ruby pods come online'
...
kubectl wait --for=condition=ready pod --timeout=90s --selector=move2kube.konveyor.io/service=python || exit
kubectl get pods
echo 'after python pods come online'
kubectl wait --for=condition=ready pod --timeout=90s --selector=move2kube.konveyor.io/service=django || exit
kubectl get pods
echo 'after django pods come online'
kubectl wait --for=condition=ready pod --timeout=90s --selector=move2kube.konveyor.io/service=java-war || exit
kubectl get pods
echo 'after java-war pods come online'
...
function all_tests() {
    ...
    check_url localhost/django   || passed=1
    check_url localhost/java-war || passed=1
    return "$passed"
}
